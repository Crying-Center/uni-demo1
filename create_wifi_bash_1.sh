...
# 交互式批量创建WiFi脚本 - 带PassWall规则创建（最终修复版）
echo "======================================"
...
echo "创建PassWall规则: $( [ "$create_passwall" = "y" ] && echo "是" || echo "否" )"
echo "--------------------------------------"
...
for i in $(seq 1 $wifi_count); do
    ...
    if [ "$create_passwall" = "y" ] || [ "$create_passwall" = "Y" ]; then
        ...
        if [ -f "/etc/config/passwall" ]; then
            # 更可靠的规则存在检查
            rule_exists=false
            pw_rules=$(uci show passwall | grep -E 'passwall\.@rule\[[0-9]+\]\.name=')
            while IFS= read -r rule_line; do
                rule_name=$(echo "$rule_line" | cut -d'=' -f2 | tr -d "'")
                if [ "$rule_name" = "$wifi_name" ]; then
                    rule_exists=true
                    break
                fi
            done <<< "$pw_rules"
            ...
            if ! $rule_exists; then
                # 获取当前规则数量
                rule_count=$(uci show passwall | grep -c 'passwall\.@rule\[[0-9]+\]\.name=')
                new_index=$rule_count
                
                # 创建新规则
                uci add passwall rule > /dev/null
                uci set "passwall.@rule[$new_index].name=$wifi_name"
                uci set "passwall.@rule[$new_index].enabled=1"
                uci set "passwall.@rule[$new_index].network=tcp udp"
                uci set "passwall.@rule[$new_index].src_ipnetwork=$subnet"
                uci set "passwall.@rule[$new_index].domain=regexp:.*"
                uci set "passwall.@rule[$new_index].ip=0.0.0.0/0"
                uci set "passwall.@rule[$new_index].remark=自动创建: $wifi_name 网络"
                
                echo "已创建PassWall规则: $wifi_name (源: $subnet)"
            else
                echo "PassWall规则 $wifi_name 已存在，跳过创建"
            fi
        else
...
if [ "$create_passwall" = "y" ]; then
    ...
    echo -e "\n验证PassWall规则配置:"
    if [ -f "/etc/config/passwall" ]; then
        grep -A 10 -E "config rule" /etc/config/passwall | grep -E "option name '$wifi_prefix"
    else
